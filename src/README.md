...
* `code/`: Complete source code to reproduce the results in the paper.
* `slurm/`:  Sample [SLURM](https://www.schedmd.com/) `sbatch` files that illustrate the usage of the code.
* `noslurm/`: Sample run scripts for programs that were not scheduled with SLURM.
