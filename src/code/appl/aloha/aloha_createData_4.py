...
with h5py.File('../../data/aloha' + str(dim) + '.h5', 'w') as ds:
    ...
    # choose Rmin around mu of ck with standard deviation sigma
    Rmin = (mu + np.random.randn(numchan, dim) * sigma) * ck
    ...
    for i in range(numchan):
        while True:
            ...
            print("updating %d" % i)
            Rmin[i] = (mu + np.random.randn(dim) * sigma) * ck[i]
