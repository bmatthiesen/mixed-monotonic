...
cdef extern from "gee_tin.h":
    ...
    cdef cppclass gee_tin`'`'DIM "GEE_TIN<DIM>":
        vtype mu
        double psi
        double alpha[DIM]
        vtype beta[DIM]
...
cdef class GEE_TIN`'`'DIM:
    cdef gee_tin`'`'DIM *_thisptr
    cdef cnp.dtype result_dt

    @wraparound(False)
    @boundscheck(False)
    def __cinit__(self, double mu, double psi, double sigma):
        self._thisptr = new gee_tin`'`'DIM`'`'()
