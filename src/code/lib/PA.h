...
class PA
{
	public:
		...
		PA()
			: output(true), output_every(100), MaxIter(-1), user_data(NULL), xopt {}, data_size((Dim+1)*sizeof(basetype)), epsilon(1e-2), proj_delta(1e-10), ub {}, obj(NULL), inG(NULL), inH(NULL), doshift(false), xshift {}
		{ setStatus(UNSOLVED); }
	...
	protected:
		...
		// functions
		virtual vtype projection(const vtype& x) const;
